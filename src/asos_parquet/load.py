...
# Station metadata columns to embed in observation rows.
# These are joined from station metadata (IEM GeoJSON) onto each observation.
STATION_METADATA_COLUMNS = ["name", "elevation", "country", "county", "wfo", "tzname"]
...
def write_year_partition(
    ...
    # Drop columns that conflict with parquet write options:
    # - 'year' is encoded in the Hive-style directory name
    # - 'bbox' is regenerated by write_covering_bbox=True
    drop = [c for c in ("year", "bbox") if c in gdf.columns]
    if drop:
...
def load_year(
    ...
    try:
        ...
        # Embed station metadata into observation rows
        observations = enrich_with_station_metadata(observations, stations)
...
def read_year_partition(
    ...
    try:
        # Use pd.read_parquet to avoid pyarrow Hive partition inference
        # that conflicts with a 'year' column inside year=XXXX directories.
...
def update_year(
    ...
    try:
        ...
        # Merge with existing data, then refresh station metadata
